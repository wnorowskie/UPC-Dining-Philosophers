...
typedef enum{THINKING, STARVING, STANDING, SITTING, EATING} philosopher_status_t;
...
void life_cycle(void)
{
  ...
  // if(check_table());
  // have check_table() check the waiter array 
  // to determine if seat is available

    if (state == STARVING){
        // missed out last seating
        if(waiter < 5){
          waiter++;
          state = SITTING;
        }
    ...
    { // no seats at the table available
      state = STARVING;
    }
  }
